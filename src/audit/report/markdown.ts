...
import type { EnrichedFinding, Severity } from "../types";
...
export function formatMarkdown(
  ...
  meta: { project: string; date: string; llm: string },
): string {
  const grouped = new Map<Severity, EnrichedFinding[]>();
  ...
  const lines: string[] = [
    ...
    `**Total:** ${findings.length} (` +
      SEVERITY_ORDER.map(
        (s) => `${SEVERITY_EMOJI[s]} ${grouped.get(s)!.length} ${s}`,
      ).join(" / ") +
      ")",
    "",
  ];
