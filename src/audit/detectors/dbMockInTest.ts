...
const DB_MOCK_RE =
  /(?:vi|jest)\s*\.\s*mock\s*\(\s*["'`]([^"'`]*(?:db\/pool|database|drizzle|prisma|knex|kysely)[^"'`]*)["'`]/g;

export const dbMockInTestDetector: Detector = {
  name: "dbMockInTest",
  ...
  async run(ctx: AuditContext): Promise<Finding[]> {
    ...
    for (const fp of files) {
      ...
      if (/\.smoke\.test\.[tj]sx?$/.test(fp)) continue;
      ...
      while ((m = DB_MOCK_RE.exec(source)) !== null) {
        ...
        findings.push({
          ...
          evidence:
            "Mockar pool/database em teste de integração esconde drift entre " +
          ...
          suggestedFix:
            "Converter pra *.smoke.test.ts usando DB real + fixtures. Ver: " +
