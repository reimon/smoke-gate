 ...
 */
export function registerVitestSuite(suite: SmokeSuite): void {
  describe(suite.name, () => {
    ...
    for (const ep of suite.endpoints) {
      ...
      it(name, async () => {
        ...
        if (fails) {
          ...
          // eslint-disable-next-line no-console
          console.error(
            `[smoke] ${ep.method} ${path} → ${res.status}`,
            res.body,
          );
        }
        ...
        if (fails) {
          throw new Error(
            `${ep.method} ${path}: status ${res.status} ∈ notStatuses [${notStatuses.join(",")}]`,
          );
        }
        ...
        if (suite.expect?.maxLatencyMs !== undefined) {
          expect(
            res.durationMs,
            `${ep.method} ${path} latency`,
          ).toBeLessThanOrEqual(suite.expect.maxLatencyMs);
        }
      });
    }
  });
 ...
 */
export function registerVitestSingleTest(suite: SmokeSuite): void {
  describe(suite.name, () => {
    it("todos os endpoints retornam status aceitável", async () => {
      ...
      if (report.failed + report.errors > 0) {
        const failed = report.results
          ...
          .map(
            (r) =>
              `  - ${r.endpoint.method} ${r.finalPath} → ${r.reason ?? "?"}`,
          )
          .join("\n");
        throw new Error(
          `${report.failed} failed, ${report.errors} errors in ${report.total}:\n${failed}`,
        );
      }
